...
namespace authorRESTAPI.Data
{
    public class CourseDAL : ICourse
    {
        ...
        {
            var result = await (from c in _db.Courses where c.Title==name select c).SingleOrDefaultAsync();
            if (result == null) throw new Exception($"Data course dengan title {name} tidak ditemukan");
            return result;
        }
